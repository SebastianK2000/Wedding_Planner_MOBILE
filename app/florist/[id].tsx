...
import { 
    ArrowLeft, Star, MapPin, Check, Flower, 
    Palette, Scissors, Share2, Heart, ShoppingCart 
} from 'lucide-react-native';
import { FloristItem } from './index';

const PLN = (n: number) => new Intl.NumberFormat("pl-PL", { style: "currency", currency: "PLN", maximumFractionDigits: 0 }).format(n);

export default function FloristDetailsScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const item: FloristItem = params.data ? JSON.parse(params.data as string) : null;
  ...
  const getIcon = (feature: string) => {
    ...
    if(f.includes("bukiet") || f.includes("wianek")) return <Flower size={18} className="text-stone-700"/>;
    if(f.includes("kolor") || f.includes("styl")) return <Palette size={18} className="text-stone-700"/>;
    if(f.includes("kompozycja") || f.includes("dekoracja")) return <Scissors size={18} className="text-stone-700"/>;
  ...
  const handleAddToCart = async () => {
      try {
          ...
          if (exists) {
              Alert.alert("Informacja", "Ta firma jest już w koszyku.");
              return;
          }

          const itemToAdd = { ...item, category: 'florist' };
        ...
        <View className="px-6 py-8 -mt-6 rounded-t-[2.5rem] bg-white">
            <View className="flex-row border-b border-stone-100 pb-8 mb-8">
                ...
                <View className="flex-1 pl-6">
                    ...
                    <Text className="text-base font-medium text-stone-900">{item.styles.slice(0, 2).join(", ")}</Text>
                </View>
            </View>
            ...
            <Text className="text-2xl font-bold text-stone-900 mb-4">Oferta dekoracji</Text>
      ...
      <SafeAreaView edges={['bottom']} className="bg-white border-t border-stone-100 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <View className="flex-row gap-4 items-center">
            <View className="flex-1">
                <Text className="text-sm text-stone-500">Koszt dekoracji</Text>
