...
import { MapPin, Star, Flower2, ShoppingBag } from 'lucide-react-native';
...
export default function FloristDetailScreen() {
  const { id } = useLocalSearchParams();
  const [data, setData] = useState<FloristDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const res = await api.get(`/florists/${id}/`);
        setData(res.data);
      } catch (e) {
        console.error(e);
  ...
  const handleAddToCart = () => {
    ...
    Alert.alert("Dodano", "Dekoracje dodane do koszyka");
  };
      ...
      <ScrollView className="flex-1">
        ...
        <View className="p-6 -mt-6 bg-stone-50 rounded-t-3xl min-h-screen">
            <View className="flex-row justify-between mb-8 border-b border-stone-200 pb-6">
                <View>
                    <Text className="text-stone-500 text-xs uppercase mb-1">Pakiety od</Text>
                ...
                <View className="items-end">
                    ...
                    <View className="flex-row items-center gap-2">
                        ...
                        <Text className="text-xl font-bold text-stone-900">Florystyka</Text>
                    </View>
                </View>
            </View>

            <Text className="text-lg font-bold text-stone-900 mb-2">O dekoracjach</Text>
