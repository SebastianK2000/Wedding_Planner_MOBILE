...
import { MapPin, Star, Car, ShoppingBag } from 'lucide-react-native';
...
export default function TransportDetailScreen() {
  ...
  const [data, setData] = useState<TransportDetail | null>(null);
  ...
  const actualPrice = data?.priceperhour || data?.price || data?.pricefrom || "0";
  ...
  const handleAddToCart = () => {
    ...
    Alert.alert("Dodano", "Transport został dodany do koszyka");
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-stone-50">
      <ScrollView className="flex-1">
        ...
        <View className="p-6 -mt-6 bg-stone-50 rounded-t-3xl min-h-screen">
            <View className="flex-row justify-between mb-8 border-b border-stone-200 pb-6">
                <View>
                    <Text className="text-stone-500 text-xs uppercase mb-1">Cena za godzinę</Text>
                    <View className="flex-row items-center gap-2">
                        <Text className="text-2xl font-bold text-stone-900">{actualPrice} zł</Text>
                    </View>
                </View>
                ...
                <View className="items-end">
                    ...
                    <View className="flex-row items-center gap-2">
                        ...
                        <Text className="text-xl font-bold text-stone-900">Transport</Text>
                    </View>
                </View>
            </View>

            <Text className="text-lg font-bold text-stone-900 mb-2">O pojeździe</Text>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white p-4 border-t border-stone-100 flex-row items-center gap-4 pb-8">
         <View>
            <Text className="text-stone-500 text-xs">Koszt / godzina</Text>
            <Text className="text-xl font-bold text-stone-900">{actualPrice} zł</Text>
         </View>
         <TouchableOpacity 
