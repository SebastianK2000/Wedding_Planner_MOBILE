...
import { 
    ArrowLeft, Star, MapPin, Check, CarFront, 
    Gauge, Users, Share2, Heart, ShoppingCart, GlassWater 
...
export default function TransportDetailsScreen() {
  ...
  const item: TransportItem = params.data ? JSON.parse(params.data as string) : null;
  ...
  const getIcon = (feature: string) => {
    ...
    if(f.includes("kierowca")) return <CarFront size={18} className="text-stone-700"/>;
    if(f.includes("szampan") || f.includes("prosecco")) return <GlassWater size={18} className="text-stone-700"/>;
    if(f.includes("dekoracja") || f.includes("dywan")) return <Star size={18} className="text-stone-700"/>;
  ...
  const handleAddToCart = async () => {
      try {
          ...
          if (exists) {
              Alert.alert("Informacja", "Ten pojazd jest już w koszyku.");
              return;
          }

          const itemToAdd = { ...item, category: 'transport' };
          ...
          <View className="absolute bottom-0 left-0 right-0 p-6">
             <View className="flex-row items-center gap-2 mb-3">
                 <View className="bg-orange-500 px-3 py-1 rounded-full">
                    <Text className="text-white text-xs font-bold uppercase tracking-wider">{item.type}</Text>
                 </View>
        ...
        <View className="px-6 py-8 -mt-6 rounded-t-[2.5rem] bg-white">
            <View className="flex-row border-b border-stone-100 pb-8 mb-8">
                <View className="flex-1 border-r border-stone-100 pr-4">
                    <Text className="text-xs text-stone-400 uppercase font-bold tracking-wider mb-1">Cena / dzień</Text>
                    <Text className="text-xl font-bold text-stone-900">{PLN(item.priceFrom)}</Text>
                </View>
                <View className="flex-1 pl-6">
                    <Text className="text-xs text-stone-400 uppercase font-bold tracking-wider mb-1">Miejsca</Text>
                    <View className="flex-row items-center gap-2">
                         <Users size={20} className="text-stone-800" />
                         <Text className="text-base font-medium text-stone-900">{item.passengers}</Text>
                    </View>
                </View>
            </View>

            <Text className="text-2xl font-bold text-stone-900 mb-4">O pojeździe</Text>
            <Text className="text-stone-600 text-lg leading-7 mb-8">{item.desc}</Text>

            <Text className="text-2xl font-bold text-stone-900 mb-4">W cenie</Text>
      ...
      <SafeAreaView edges={['bottom']} className="bg-white border-t border-stone-100 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <View className="flex-row gap-4 items-center">
            <View className="flex-1">
                <Text className="text-sm text-stone-500">Koszt wynajmu</Text>
            ...
            >
                {inCart ? (
                    ...
                    <>
                        ...
                        <Text className="text-white font-bold text-lg">Zarezerwuj</Text>
                    </>
                )}
            </TouchableOpacity>
        </View>
