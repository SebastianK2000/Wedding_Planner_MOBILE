      ...
      try {
        ...
        if (tasksRes.status === "fulfilled") {
           ...
           if (Array.isArray(tasks) && tasks.length > 0) {
              const salaTask = tasks.find((t: any) => t.title?.toLowerCase().includes("sala") || t.title?.toLowerCase().includes("lokal")) as any;
              const isDone = salaTask && salaTask.status === "done" ? true : false;
    ...
    <ScrollView className="bg-white flex-1">
      ...
      <SafeAreaView className="flex-1 px-4 py-8">
        <View className="flex flex-col gap-10">
            ...
            <View className="w-full flex flex-col gap-3">
              <View className="relative w-full">
                ...
                <TextInput
                  ...
                  placeholder="Data ślubu (RRRR-MM-DD)"
                />
              </View>
          ...
          <View className="relative w-full aspect-[4/5] rounded-[2.5rem] bg-stone-200 shadow-2xl overflow-hidden mt-4 mb-12">
            <Image
            ...
            <View className="absolute top-6 left-4 bg-white/90 p-3 rounded-2xl shadow-xl border border-white/50 backdrop-blur-md flex-row items-center gap-3 w-48">
                ...
                <View>
                  <Text className="text-sm font-bold text-stone-800">Sala</Text>
                  <Text className="text-xs text-stone-500">
                    {taskStatus.hasData 
                        ? (taskStatus.salaDone ? "Ukończone" : "Do zrobienia") 
                        : "Zaliczka"}
                  </Text>
                </View>
            </View>
            ...
            <View className="absolute bottom-24 right-4 bg-white/90 p-3 rounded-2xl shadow-xl border border-white/50 backdrop-blur-md flex-row items-center gap-3 w-48">
                ...
                <View className="flex-1">
                  <Text className="text-sm font-bold text-stone-800">Budżet</Text>
