...
import { 
    ArrowLeft, Star, MapPin, Check, Music, 
    Mic2, Speaker, Share2, Heart, ShoppingCart, Disc 
...
export default function MusicDetailsScreen() {
  ...
  const item: MusicItem = params.data ? JSON.parse(params.data as string) : null;
  ...
  const getIcon = (feature: string) => {
    ...
    if(f.includes("wodzirej") || f.includes("śpiew")) return <Mic2 size={18} className="text-stone-700"/>;
    if(f.includes("dym") || f.includes("światł")) return <Disc size={18} className="text-stone-700"/>;
    if(f.includes("saksofon") || f.includes("skrzypce")) return <Music size={18} className="text-stone-700"/>;
  ...
  const handleAddToCart = async () => {
      try {
          ...
          if (exists) {
              Alert.alert("Informacja", "Ten wykonawca jest już w koszyku.");
              return;
          }

          const itemToAdd = { ...item, category: 'music' };
        ...
        <View className="px-6 py-8 -mt-6 rounded-t-[2.5rem] bg-white">
            <View className="flex-row border-b border-stone-100 pb-8 mb-8">
                <View className="flex-1 border-r border-stone-100 pr-4">
                    <Text className="text-xs text-stone-400 uppercase font-bold tracking-wider mb-1">Cena od</Text>
                    <Text className="text-xl font-bold text-stone-900">{PLN(item.priceFrom)}</Text>
                </View>
                <View className="flex-1 pl-6">
                    <Text className="text-xs text-stone-400 uppercase font-bold tracking-wider mb-1">Gatunki</Text>
                    <Text className="text-base font-medium text-stone-900">{item.genres.slice(0, 2).join(", ")}</Text>
                </View>
            </View>

            <Text className="text-2xl font-bold text-stone-900 mb-4">O nas</Text>
            <Text className="text-stone-600 text-lg leading-7 mb-8">{item.desc}</Text>

            <Text className="text-2xl font-bold text-stone-900 mb-4">Oferta i sprzęt</Text>
      ...
      <SafeAreaView edges={['bottom']} className="bg-white border-t border-stone-100 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        <View className="flex-row gap-4 items-center">
            <View className="flex-1">
                <Text className="text-sm text-stone-500">Koszt całkowity</Text>
